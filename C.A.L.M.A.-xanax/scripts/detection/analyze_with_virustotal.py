...
def load_config(config_file: str = None) -> dict:
    ...
    if not Path(config_file).exists():
        print(f"[ERRO] Ficheiro de config nao encontrado: {config_file}")
        sys.exit(1)
...
def analyze_and_notify(file_path: str, config_file: str = None, 
    ...
    if not file_path.exists():
        print(f"[ERRO] Ficheiro nao encontrado: {file_path}")
        return 1
    ...
    try:
        ...
        if use_vt:
            print("[INFO] Iniciando analise via VirusTotal (sandbox)...\n")
        else:
            print("[INFO] VirusTotal nao configurado. Usando analise local...\n")
        ...
        status_tag = "[LIMPO]" if result.prediction == "LIMPO" else \
                     "[SUSPEITO]" if result.prediction == "SUSPEITO" else "[INFECTADO]"
        
        print(f"{status_tag} Resultado: {result.prediction}")
        ...
        if result.virustotal_result and not result.virustotal_result.error:
            ...
            print(f"\nVirusTotal Detalhes:")
            print(f"   Deteccoes: {vt.malicious_count}/{vt.total_vendors} antivirus")
            if vt.threat_name:
        ...
        if send_email and config.get('notifications_enabled', True):
            print(f"\n[INFO] Enviando notificacao por email...")
            
            try:
                notifier = NotificationService(config_file)
                if notifier.send_detection_notification(result, filename=file_path.name):
                    print(f"[OK] Notificacao enviada com sucesso!")
                else:
                    print(f"[AVISO] Notificacao nao foi enviada (desabilitada para esta classificacao)")
            except Exception as e:
                print(f"[AVISO] Erro ao enviar notificacao: {e}")
