...
echo -n "6. Configuracao... "
if [ -f "config/calma_config.json" ]; then
    if grep -q "your_email@gmail.com\|seu_email@gmail.com" config/calma_config.json 2>/dev/null; then
        echo -e "${YELLOW}[!]${NC} Configuracao padrao (execute python setup.py)"
        ((WARNINGS++))
    else
        echo -e "${GREEN}[OK]${NC} (config/calma_config.json configurado)"
    fi
else
    echo -e "${YELLOW}[!]${NC} config/calma_config.json nao encontrado"
    ((WARNINGS++))
fi
